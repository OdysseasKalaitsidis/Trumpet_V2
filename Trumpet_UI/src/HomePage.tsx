...
export default function HomePage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        switch (pathFilter) {
          case "ArtMusic":
            title = "Η μουσική του άστεως (Art Music)";
            break;
          case "UrbanPopular":
            title = "Η αστικολαϊκή μουσική (Urban Popular Music)";
            break;
          case "RuralMusic":
            title = "Η μουσική της υπαίθρου (Rural Music)";
            break;
          case "SacredMusic":
            title = "Η εκκλησιαστική μουσική (Sacred Music)";
            break;
        }
  ...
  const getCoverUrl = (item: Item) => {
    ...
    const imageBitstreams = item.bitstreams.filter(
      (b) =>
    ...
    const pdf = item.bitstreams.find(
      (b) => b.mimeType === "application/pdf" || b.name.endsWith(".pdf")
    );
    if (pdf) {
    ...
    <>
      <div className="text-center mb-12">
        ...
        <p className="text-xl text-white/75">
          Select a Music Path to explore the archive.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between mb-8 border-b border-white/10 pb-4">
        ...
        {pathFilter && (
          <Link
            to="/"
            className="btn-liquid border border-white/20 hover:bg-white/10 text-white no-underline text-sm"
          >
            Show All Archives
          </Link>
        )}
      ...
      <p className="text-white/60 mb-8">
        Found {items.length} items in this path.
      </p>
            ...
            items.map((item) => {
              ...
              return (
                <div className="col-span-1" key={item.id}>
                  <Link
                    to={`/item/${item.id}`}
                    className="text-decoration-none text-white block h-full"
                  >
                    <div className="card-glass h-full item-card flex flex-col">
                        ...
                        <>
                          <img
                            ...
                            onError={(e) => {
                              ...
                              console.error("Image load failed:", coverUrl);
                            }}
                          />
                      ...
                      <div className="p-4 flex-1">
                        ...
                        <p className="text-sm text-white/60 truncate">
                          {item.metadata.find(
                            (m) => m.field === "dc.contributor.author"
                          )?.value || ""}
                        </p>
                      </div>
                    </div>
